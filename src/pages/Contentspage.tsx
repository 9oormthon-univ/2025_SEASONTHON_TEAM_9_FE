...
import { TokenReq } from "../apis/axiosInstance"; // 경로 맞게 import
...
export default function Contentspage() {
    ...
    return (!select ? <Container>
        ...
        <ContentsContainer>
            ...
            <Gridcontainer>
                {contentsdata.map((items, index) => (
                    <div style={{ flexDirection: "column", display: "flex", height: "250px" }} key={index}>
                        <Element_img
                            onClick={() => {
                                setdetaildatas({
                                    title: items.title,
        ...
        : <Container>
            <Detail_container>
                <Backbar><div style={{ marginRight: "20px" }} onClick={() => { setselect(false) }}>{"<"}</div></Backbar>
                <Detail_title>
                    <text>{detaildatas?.title}</text>
                    <img src={detaildatas?.bookmarking ? bookmark_fill : bookmark_default} style={{ position: "absolute", right: "10px" }}></img>
                </Detail_title>
                <Detail_writer>{detaildatas?.writer}</Detail_writer>
                <div style={{ width: "100%", marginTop: "10px", flexDirection: "row", display: "flex", backgroundColor: "" }}>
                    <Detail_date>{detaildatas?.date}</Detail_date>
                    <Detail_tags>{detaildatas?.tags.map((items, index) => (<Element_tags key={index}>{items}</Element_tags>))}</Detail_tags>
                </div>
                <div style={{ width: "100%", height: "20px", borderBottom: "1px solid #DFE1E5" }}></div>
                <Detail_image></Detail_image>
                <Detail_discription>
                    {detaildatas?.discription}
                </Detail_discription>
